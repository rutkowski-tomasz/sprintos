...
-- sprints
CREATE POLICY "sprints: select own" ON sprints FOR SELECT USING ("userId" = auth.uid());
CREATE POLICY "sprints: insert own" ON sprints FOR INSERT WITH CHECK ("userId" = auth.uid());
CREATE POLICY "sprints: update own" ON sprints FOR UPDATE USING ("userId" = auth.uid());
CREATE POLICY "sprints: delete own" ON sprints FOR DELETE USING ("userId" = auth.uid());
